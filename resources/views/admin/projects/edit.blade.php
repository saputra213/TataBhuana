...
@section('title', 'Edit Proyek - Admin Panel')
...
<div class="row">
    <div class="col-lg-8">
        <div class="card">
            <div class="card-header">
                <h5 class="mb-0">Form Edit Proyek</h5>
            </div>
            <div class="card-body">
                <form action="{{ route('admin.projects.update', $project) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="form-select @error('project_type') is-invalid @enderror" id="project_type" name="project_type" required>
                                ...
                                <option value="konstruksi" {{ old('project_type', $project->project_type) == 'konstruksi' ? 'selected' : '' }}>Konstruksi</option>
                                <option value="renovasi" {{ old('project_type', $project->project_type) == 'renovasi' ? 'selected' : '' }}>Renovasi</option>
                                <option value="maintenance" {{ old('project_type', $project->project_type) == 'maintenance' ? 'selected' : '' }}>Maintenance</option>
                                <option value="event" {{ old('project_type', $project->project_type) == 'event' ? 'selected' : '' }}>Event</option>
                            </select>
                            @error('project_type')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="number" class="form-control @error('sort_order') is-invalid @enderror" id="sort_order" name="sort_order" value="{{ old('sort_order', $project->sort_order) }}" min="0">
                            @error('sort_order')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                            ...
                            <div class="mb-2">
                                ...
                                <div class="row g-2">
                                    @foreach($project->images as $image)
                                        <div class="col-3">
                                            <img src="{{ asset('storage/' . $image) }}" alt="Gambar proyek" class="img-thumbnail" style="width: 100%; height: 80px; object-fit: cover;">
                                        </div>
                                    @endforeach
                                </div>
                            </div>
                    ...
                    <div class="mb-3 form-check">
                        <input type="checkbox" class="form-check-input" id="is_featured" name="is_featured" {{ old('is_featured', $project->is_featured) ? 'checked' : '' }}>
    ...
    <div class="col-lg-4">
        <div class="card">
            <div class="card-header">
                <h6 class="mb-0">Informasi Proyek</h6>
            </div>
            <div class="card-body">
